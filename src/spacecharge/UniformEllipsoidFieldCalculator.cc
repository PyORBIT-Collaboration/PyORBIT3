/**
 Calculates the electric field generated by a uniformly charged ellipsoid in free space.

 - 3D fields: calculated from symmetric elliptic integrals, evaluated using Carlson's formulas.
 - 2D transverse fields (long-bunch approximation): calculated from analytic formulas.
 - 1D longitudinal fields (long-bunch approximation): to do.
 **/
...
UniformEllipsoidFieldCalculator::UniformEllipsoidFieldCalculator() : CppPyWrapper(NULL) {
  ...
  // Number of points for lambda function
  ...
  // Ellipse parameters (x/a)^2 + (y/b)^2 + (z/c)^2 = 1
  a = 1.0;
  b = 1.0;
  c = 1.0;
  double r_max = 10.0;
...
/** Sets ellipsoid semi-axes (a, b, c). */
void UniformEllipsoidFieldCalculator::setEllipsoid(double a_in, double b_in, double c_in, double r_max) {
  ...
  // Integrate the part from lambda to lambda_max for different lambda and put values into functions.
...
/** Sets ellipsoid semi-axes in transverse plane (a, b). */
void UniformEllipsoidFieldCalculator::setEllipse(double a_in, double b_in) {
  a = a_in;
  b = b_in;
  a2 = a * a;
  b2 = b * b;
...
void UniformEllipsoidFieldCalculator::calcField(double x, double y, double z, double x2, double y2, double z2, double &ex, double &ey, double &ez) {
...
/** Calculates the field components of a uniformly charged ellipsoid. */
void UniformEllipsoidFieldCalculator::calcField2D(double x, double y, double x2, double y2, double &ex, double &ey) {
  double lambda = 0.;
  if ((x2 / a2 + y2 / b2) > 1.) {
    double p = a2 + b2 - x2 - y2;
    double q = a2 * b2 - x2 * b2 - y2 * a2;
    double discriminant = p * p - 4. * q;
    if (discriminant < 0.)
      discriminant = 0.;
    lambda = 0.5 * (-p + sqrt(discriminant));
  }

  double a_lambda = sqrt(a2 + lambda);
  double b_lambda = sqrt(b2 + lambda);
  ex = 2. * Q_total * x / (a_lambda * (a_lambda + b_lambda));
  ey = 2. * Q_total * y / (b_lambda * (a_lambda + b_lambda));
}

/** Calculates lambda value: root of x^2/(a^2+s) + y^2/(b^2+s) + z^2/(c^2+s) - 1 = 0 */
double UniformEllipsoidFieldCalculator::calcLambda(double x, double y, double z, double x2, double y2, double z2) {
  double lambda_start = 0.;
  double a2_ls, b2_ls, c2_ls;
  a2_ls = a2 + lambda_start;
  b2_ls = b2 + lambda_start;
  c2_ls = c2 + lambda_start;
...
/** Returns the total space charge inside the ellipsoid. */
double UniformEllipsoidFieldCalculator::getQ() {
  return Q_total;
}

/** Sets the total space charge inside the ellipsoid. */
