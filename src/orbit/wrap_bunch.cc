...
static int Bunch_init(pyORBIT_Object *self, PyObject *args, PyObject *kwds)
{
  ...
  PyObject *pySyncPart =
    PyObject_CallMethod(mod, const_cast<char *>("SyncParticle"), const_cast<char *>("O"), self);
...
static PyObject *Bunch_x(PyObject *self, PyObject *args)
{
  ...
  if (!PyArg_ParseTuple(args, "i|d:x", &index, &value)) {
    return NULL;
  }

  if (PyTuple_GET_SIZE(args) == 1) {
    value = cpp_bunch->x(index);
  }
  else {
    cpp_bunch->x(index) = value;
  }
...
static PyObject *Bunch_y(PyObject *self, PyObject *args)
{
  ...
  if (!PyArg_ParseTuple(args, "i|d:y", &index, &value)) {
    return NULL;
  }

  if (PyTuple_GET_SIZE(args) == 1) {
    value = cpp_bunch->y(index);
  }
  else {
    cpp_bunch->y(index) = value;
  }
...
static PyObject *Bunch_z(PyObject *self, PyObject *args)
{
  ...
  if (!PyArg_ParseTuple(args, "i|d:z", &index, &value)) {
    return NULL;
  }

  if (PyTuple_GET_SIZE(args) == 1) {
    value = cpp_bunch->z(index);
  }
  else {
    cpp_bunch->z(index) = value;
  }
...
static PyObject *Bunch_px(PyObject *self, PyObject *args)
{
  ...
  if (!PyArg_ParseTuple(args, "i|d:px", &index, &value)) {
    return NULL;
  }

  if (PyTuple_GET_SIZE(args) == 1) {
    value = cpp_bunch->px(index);
  }
  else {
    cpp_bunch->px(index) = value;
  }
...
static PyObject *Bunch_py(PyObject *self, PyObject *args)
{
  ...
  if (!PyArg_ParseTuple(args, "i|d:py", &index, &value)) {
    return NULL;
  }

  if (PyTuple_GET_SIZE(args) == 1) {
    value = cpp_bunch->py(index);
  }
  else {
    cpp_bunch->py(index) = value;
  }
...
static PyObject *Bunch_pz(PyObject *self, PyObject *args)
{
  ...
  if (!PyArg_ParseTuple(args, "i|d:pz", &index, &value)) {
    return NULL;
  }

  if (PyTuple_GET_SIZE(args) == 1) {
    value = cpp_bunch->pz(index);
  }
  else {
    cpp_bunch->pz(index) = value;
  }
...
static PyObject *Bunch_mass(PyObject *self, PyObject *args)
{
  ...
  if (PyTuple_GET_SIZE(args) == 0) {
    return PyFloat_FromDouble(cpp_bunch->getMass());
  }
...
static PyObject *Bunch_charge(PyObject *self, PyObject *args)
{
  ...
  if (PyTuple_GET_SIZE(args) == 0) {
    return PyFloat_FromDouble(cpp_bunch->getCharge());
  }

  double value;
  if (!PyArg_ParseTuple(args, "d:charge", &value)) {
    return NULL;
  }

  cpp_bunch->setCharge(value);
  return PyFloat_FromDouble(value);
}
...
static PyObject *Bunch_macroSize(PyObject *self, PyObject *args)
{
  ...
  if (PyTuple_GET_SIZE(args) == 0) {
    return PyFloat_FromDouble(cpp_bunch->getMacroSize());
  }

  double value;
  if (!PyArg_ParseTuple(args, "d:macroSize", &value)) {
    return NULL;
  }

  cpp_bunch->setMacroSize(value);
  return PyFloat_FromDouble(value);
}
...
static PyObject *Bunch_bunchAttrDouble(PyObject *self, PyObject *args)
{
  ...
  if (!PyArg_ParseTuple(args, "s|d:bunchAttrDouble", &attr_name, &value)) {
    return NULL;
  }

  std::string attr_name_str(attr_name);
  if (PyTuple_GET_SIZE(args) == 1) {
    value = cpp_bunch->getBunchAttributeDouble(attr_name_str);
  }
  else {
    cpp_bunch->setBunchAttribute(attr_name_str, value);
  }
...
static PyObject *Bunch_bunchAttrInt(PyObject *self, PyObject *args)
{
  ...
  if (!PyArg_ParseTuple(args, "s|i:bunchAttrInt", &attr_name, &value)) {
    return NULL;
  }

  std::string attr_name_str(attr_name);
  if (PyTuple_GET_SIZE(args) == 1) {
    value = cpp_bunch->getBunchAttributeInt(attr_name_str);
  }
  else {
    cpp_bunch->setBunchAttribute(attr_name_str, value);
  }
  ...
  if (py_attrParamsDict != NULL) {
    ...
    while (PyDict_Next(py_attrParamsDict, &pos, &key, &value)) {
      if (!PyUnicode_Check(key) || !PyNumber_Check(value)) {
        error(
          "PyBunch - addPartAttr(name, [param_dict]) - param_dict is not a {str:val} dictionary"
        );
      }
...
static PyObject *Bunch_getPartAttrDicts(PyObject *self, PyObject *Py_UNUSED(ignored))
{
  ...
  for (int i = 0, n = names.size(); i < n; i++) {
    ...
    if (py_param_dict == NULL) {
      Py_DECREF(resDict);
      return NULL;
    }
    if (PyDict_SetItemString(resDict, names[i].c_str(), py_param_dict) < 0) {
      Py_DECREF(py_param_dict);
      Py_DECREF(resDict);
      return NULL;
    }
    std::map<std::string, double> param_dict =
      cpp_bunch->getParticleAttributes(names[i])->parameterDict;
    ...
    for (pos = param_dict.begin(); pos != param_dict.end(); ++pos) {
      ...
      if (py_val == NULL || PyDict_SetItemString(py_param_dict, key.c_str(), py_val) < 0) {
        Py_XDECREF(py_val);
        Py_DECREF(py_param_dict);
        Py_DECREF(resDict);
        return NULL;
...
static PyObject *Bunch_readPartAttrNames(PyObject *self, PyObject *arg)
{
  ...
  for (int i = 0, n = names.size(); i < n; i++) {
    ...
    if (PyTuple_SetItem(resTuple, i, py_nm)) {
      error(
        "PyBunch - readPartAttrNames(fileName) - cannot create tuple with particles attr. names"
      );
    }
  }
  return resTuple;
...
static PyObject *Bunch_partAttrValue(PyObject *self, PyObject *args)
{
  ...
  if (
    !PyArg_ParseTuple(args, "sii|d:partAttrValue", &attr_name, &part_index, &attr_index, &value)
  ) {
    return NULL;
  }
  ...
  if (part_index >= bunch_size || attr_size <= attr_index) {
    error(
      "PyBunch - partAttrValue(attr_name,part_index,atr_index,[val]) - indexes out of limits! "
      "Stop!"
    );
  }

  if (PyTuple_GET_SIZE(args) == 3) {
    value = cpp_bunch->getParticleAttributes(attr_name_str)->attValue(part_index, attr_index);
  }
  else {
    cpp_bunch->getParticleAttributes(attr_name_str)->attValue(part_index, attr_index) = value;
  }
...
{
  Bunch *cpp_bunch = (Bunch *)((pyORBIT_Object *)self)->cpp_obj;
  return PyLong_FromLong(cpp_bunch->getSize());
}
...
{
  Bunch *cpp_bunch = (Bunch *)((pyORBIT_Object *)self)->cpp_obj;
  return PyLong_FromLong(cpp_bunch->getSizeGlobal());
}
...
{
  Bunch *cpp_bunch = (Bunch *)((pyORBIT_Object *)self)->cpp_obj;
  return PyLong_FromLong(cpp_bunch->getSizeGlobalFromMemory());
}
...
{
  Bunch *cpp_bunch = (Bunch *)((pyORBIT_Object *)self)->cpp_obj;
  return PyLong_FromLong(cpp_bunch->getTotalCount());
}
...
{
  Bunch *cpp_bunch = (Bunch *)((pyORBIT_Object *)self)->cpp_obj;
  return PyLong_FromLong(cpp_bunch->getCapacity());
}
  ...
  if (nVars == 1 || nVars == 2) {
    if (nVars == 1) {
      ...
      if (!PyArg_ParseTuple(args, "si:read", &file_name, &nParts)) {
        error(
          "PyBunch - readBunch(fileName,nParts) - file name, and number of particles are needed"
        );
      }
      cpp_bunch->initBunchAttributes(file_name);
      cpp_bunch->readParticleAttributes(file_name);
...
static PyArrayObject *parse_bunch_array(PyObject *input)
{
  ...
  if (PyArray_NDIM(array) != 2) {
    PyErr_SetString(PyExc_ValueError, "array must be 2-dimensional with shape (nparts, 6)");
    Py_DECREF(array);
    return NULL;
  }

  if (PyArray_DIM(array, 1) != 6) {
    PyErr_SetString(PyExc_ValueError, "expected shape (nparts, 6) for (x, xp, y, yp, z, dE)");
    Py_DECREF(array);
    return NULL;
  }
...
static void append_bunch_with_PyArray(Bunch *cpp_bunch, PyArrayObject *array)
{
  ...
  for (npy_intp i = 0; i < nparts; ++i) {
    ...
    cpp_bunch->addParticle(coords[0], coords[1], coords[2], coords[3], coords[4], coords[5]);
  }
}
...
static PyObject *Bunch_to_numpy(PyObject *self, PyObject *args, PyObject *kwargs)
{
  ...
  const int rank = cpp_bunch->getMPI_Rank();
  const int size = cpp_bunch->getMPI_Size();
  ...
  if (gather && size > 1) {
    ...
    ORBIT_MPI_Allreduce(
      ...
      MPI_INT,
      MPI_MIN,
      cpp_bunch->getMPI_Comm_Local()->comm
    );
...
#if USE_MPI > 0
  MPI_Comm comm = cpp_bunch->getMPI_Comm_Local()->comm;
  ...
  if (
    MPI_Gather(&local_nparts, 1, MPI_INT, global_nparts.data(), 1, MPI_INT, root, comm) !=
    MPI_SUCCESS
  ) {
    Py_DECREF(local_array);
    PyErr_SetString(
      PyExc_RuntimeError,
      "MPI_Gather failed to collect the bunch sizes across ranks"
    );
    return NULL;
  }
...
static PyObject *Bunch_update_from_numpy(PyObject *self, PyObject *arg)
{
  ...
  cpp_bunch->deleteAllParticles();
  append_bunch_with_PyArray(cpp_bunch, array);
...
static PyObject *Bunch_from_numpy(PyObject *cls, PyObject *arg)
{
  ...
  Bunch *cpp_bunch = (Bunch *)((pyORBIT_Object *)py_bunch_obj)->cpp_obj;
...
static PyMethodDef BunchClassMethods[] = {
  ...
  {"deleteParticle",
   Bunch_deleteParticle,
   METH_O,
   "Removes macro-particle from the bunch and call compress inside"},
  {"deleteParticleFast",
   Bunch_deleteParticleFast,
   METH_O,
   "Removes macro-particle from the bunch very fast"},
  {"recoverParticle", Bunch_recoverParticle, METH_O, "Recovers a particle marked for removal"},
  {"deleteAllParticles",
   Bunch_deleteAllParticles,
   METH_NOARGS,
   "Removes all macro-particles from the bunch"},
  ...
  {"classicalRadius",
   Bunch_classicalRadius,
   METH_NOARGS,
   "Returns a classical radius of particle in [m]"},
  {"B_Rho", Bunch_B_Rho, METH_NOARGS, "Returns B*Rho parameter of particle in [T*m]"},
  {"charge",
   Bunch_charge,
   METH_VARARGS,
   "Set charge(value) or get charge() the charge of particle in e-charge"},
  {"macroSize",
   Bunch_macroSize,
   METH_VARARGS,
   "Set macroSize(value) or get macroSize() the charge of particle in e-charge"},
  {"initBunchAttr",
   Bunch_initBunchAttr,
   METH_O,
   "Reads and initilizes bunch attributes from a bunch file"},
  {"bunchAttrDouble",
   Bunch_bunchAttrDouble,
   METH_VARARGS,
   "Returns and sets a double bunch attribute"},
  {"bunchAttrInt", Bunch_bunchAttrInt, METH_VARARGS, "Returns and sets an integer bunch attribute"},
  {"bunchAttrDoubleNames",
   Bunch_bunchAttrDoubleNames,
   METH_NOARGS,
   "Returns a list of double bunch attribute names"},
  {"bunchAttrIntNames",
   Bunch_bunchAttrIntNames,
   METH_NOARGS,
   "Returns a list of integer bunch attribute names"},
  {"hasBunchAttrDouble",
   Bunch_hasBunchAttrDouble,
   METH_O,
   "Returns 1 if there is a double bunch attr. with this name, 0 - otherwise"},
  {"hasBunchAttrInt",
   Bunch_hasBunchAttrInt,
   METH_O,
   "Returns 1 if there is a int bunch attr. with this name, 0 - otherwise"},
  {"addPartAttr", Bunch_addPartAttr, METH_VARARGS, "Adds a particles' attributes to the bunch"},
  {"removePartAttr",
   Bunch_removePartAttr,
   METH_O,
   "Removes a particles' attributes from the bunch"},
  {"removeAllPartAttr",
   Bunch_removeAllPartAttr,
   METH_NOARGS,
   "Removes all particles' attributes from the bunch"},
  {"getPartAttrNames",
   Bunch_getPartAttrNames,
   METH_NOARGS,
   "Returns all particles' attributes names in the bunch at this moment"},
  {"getPartAttrDicts",
   Bunch_getPartAttrDicts,
   METH_NOARGS,
   "Returns dict{part. attribute name:dict{key:val}}"},
  {"getPossiblePartAttrNames",
   Bunch_getPossiblePartAttrNames,
   METH_NOARGS,
   "Returns all possible particles' attributes names"},
  {"clearAllPartAttrAndMemorize",
   Bunch_clearAllPartAttrAndMemorize,
   METH_NOARGS,
   "Temporary removes and memorizes all particles' attributes names"},
  {"restoreAllPartAttrFromMemory",
   Bunch_restoreAllPartAttrFromMemory,
   METH_NOARGS,
   "Restores all particles' attributes names from memory"},
  {"hasPartAttr",
   Bunch_hasPartAttr,
   METH_O,
   "Returns 1 if there is a particles' attr. with this name, 0 - otherwis"},
  {"readPartAttrNames",
   Bunch_readPartAttrNames,
   METH_O,
   "Returns a tuple with particles' attr. names in the bunch file"},
  {"readPartAttrDicts",
   Bunch_readPartAttrDicts,
   METH_O,
   "Returns a dict{attr_name:dicts{param_name:val}} in the bunch file"},
  {"readPartAttr",
   Bunch_readPartAttr,
   METH_O,
   "Initializes the particles' attr. from the bunch file"},
  {"getPartAttrSize",
   Bunch_getPartAttrSize,
   METH_O,
   "Returns the number of variables in the particles' attributes with a particular name"},
  {"partAttrValue",
   Bunch_partAttrValue,
   METH_VARARGS,
   "Sets or returns a particles' attribute value"},
  {"getSize", Bunch_getSize, METH_NOARGS, "Returns number of macro-particles"},
  {"getSizeGlobal",
   Bunch_getSizeGlobal,
   METH_NOARGS,
   "Returns number of macro-particles in all CPUs"},
  {"getSizeGlobalFromMemory",
   Bunch_getSizeGlobalFromMemory,
   METH_NOARGS,
   "Returns number of macro-particles in all CPUs from memory"},
  {"getTotalCount",
   Bunch_getTotalCount,
   METH_NOARGS,
   "Returns number of all particles - alive,dead,new"},
  {"getCapacity", Bunch_getCapacity, METH_NOARGS, "Returns the capacity of the bunch-contaiter"},
  {"dumpBunch",
   Bunch_dumpBunch,
   METH_VARARGS,
   "Prints the bunch info into a standart output stream or file"},
  {"readBunch", Bunch_readBunch, METH_VARARGS, "Reads the bunch info from a file"},
  {"copyEmptyBunchTo",
   Bunch_copyEmptyBunchTo,
   METH_O,
   "Copy bunch attrubutes and structure to another bunch"},
  {"copyBunchTo",
   Bunch_copyBunchTo,
...
static PyMemberDef BunchClassMembers[] = {{NULL}};
...
static struct PyModuleDef cModPyDem =
  {PyModuleDef_HEAD_INIT, "bunch", "Bunch class", -1, BunchModuleMethods};
...
PyMODINIT_FUNC initbunch(void)
{
  ...
  if (PyType_Ready(&pyORBIT_Bunch_Type) < 0)
    return NULL;
