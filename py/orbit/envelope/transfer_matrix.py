...
def build_unnorm_matrix_from_eigvecs(v1: np.ndarray, v2: np.ndarray) -> np.ndarray:
    V = np.zeros((4, 4))
    V[:, 0] = +np.real(v1)
    V[:, 1] = -np.imag(v1)
    V[:, 2] = +np.real(v2)
...
def build_unnorm_matrix_from_tmat(M: np.ndarray) -> np.ndarray:
...
def build_unnorm_matrix_from_params_cs(
    alpha_x: float,
    beta_x: float,
    alpha_y: float,
    beta_y: float,
    ...
    V = np.zeros((4, 4))
    V[0:2, 0:2] = build_unnorm_matrix_from_params_cs_2d(alpha_x, beta_x)
    V[2:4, 2:4] = build_unnorm_matrix_from_params_cs_2d(alpha_y, beta_y)
    return V


def build_unnorm_matrix_from_params_lb_one_mode(
    alpha_lx: float,
    beta_lx: float,
    alpha_ly: float,
    beta_ly: float,
...
def build_norm_matrix_from_tmat(*args, **kwargs) -> np.ndarray:
    return np.linalg.inv(build_unnorm_matrix_from_tmat(*args, **kwargs))


def build_norm_matrix_from_params_cs_2d(*args, **kwargs) -> np.ndarray:
    return np.linalg.inv(build_unnorm_matrix_from_params_cs_2d(*args, **kwargs))
...
def calc_params_from_tmat_lb(M: np.ndarray) -> dict[str, float]:
    V_inv = build_unnorm_matrix_from_tmat(M)
