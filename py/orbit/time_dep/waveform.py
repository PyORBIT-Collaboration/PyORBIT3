...
class SquareRootWaveform:
	...
	"""
## Pranab 2022/3/14. This seems 1-Sqrt(t/T)
## For Anti-corr painting: it's Sqrt(1-t/T) at J-PARC
...
class SquareRootWaveformAnti:
	"""
	WF for Anti Correlated painting by Y' as Sqrt(1-t/T).Pranab 3/14, 2022
	"""
	def __init__(self, syncpart, lattlength, ti, tf, si, sf):
		self.name = "square root waveform for Anti-Correlated painting"
		self.syncpart = syncpart
		self.lattlength = lattlength
		self.ti = ti
		self.tf = tf
...
#			strength = math.sqrt((1.0 - dt) + self.sf)
			dt = math.sqrt(1.0 - ((time - self.ti) / (self.tf - self.ti)))
			strength = dt + self.sf

		return strength

class SquareRootWaveformCorr:
	"""
	WF for Correlated painting by Y' as Sqrt(t/T). Pranab 3/14, 2022
	"""
	def __init__(self, syncpart, lattlength, ti, tf, si, sf):
		self.name = "square root waveform for Correlated painting"
		self.syncpart = syncpart
		self.lattlength = lattlength
		self.ti = ti
		self.tf = tf
...
class JPARC_21to27:
	...
	def getStrength(self):
		tsp = self.syncpart.time()
		if(tsp <= self.time[0]):
			strength = 1.0 + self.dQ[0]
			return strength
		if(tsp >= self.time[len(self.time) - 1]):
			strength = 1.0 + self.dQ[len(self.time) - 1]
			return strength
		for i in range(len(self.time) - 1):
			if(tsp < self.time[i + 1]):
			   tp = self.time[i + 1]
			   tm = self.time[i]
			   dt = (tsp - tm) / (tp - tm)
			   dQVal = self.dQ[i] + \
			   dt * (self.dQ[i + 1] - self.dQ[i])
			   strength = 1.0 + dQVal
			   return strength
...
class JPARC_21:
	...
	"""
	def __init__(self, syncpart, lattlength, QFLNoise):
		self.name = "JPARC_21"
		self.syncpart = syncpart
		self.lattlength = lattlength
		#self.QFLNoise = QFLNoise
		
	def getStrength(self):
		strength = 1.0 + self.QFLNoise
		#print ("QFLNoise=", self.QFLNoise)
		return strength
...
class JPARC_22:
	...
	"""
	def __init__(self, syncpart, lattlength, QDLNoise):
		self.name = "JPARC_22"
		self.syncpart = syncpart
		self.lattlength = lattlength
		#self.QDLNoise = QDLNoise
		
	def getStrength(self):
		strength = 1.0 + self.QDLNoise
		return strength
...
class JPARC_23:
	...
	"""
	def __init__(self, syncpart, lattlength, QFMNoise):
		self.name = "JPARC_23"
		self.syncpart = syncpart
		self.lattlength = lattlength
		#self.QFMNoise = QFMNoise
		
	def getStrength(self):
		strength = 1.0 + self.QFMNoise
		return strength
...
class JPARC_24:
	...
	"""
	def __init__(self, syncpart, lattlength, QDXNoise):
		self.name = "JPARC_24"
		self.syncpart = syncpart
		self.lattlength = lattlength
		#self.QDXNoise = QDXNoise
		
	def getStrength(self):
		strength = 1.0 + self.QDXNoise
		return strength
...
class JPARC_25:
	...
	"""
	def __init__(self, syncpart, lattlength, QFNNoise):
		self.name = "JPARC_25"
		self.syncpart = syncpart
		self.lattlength = lattlength
		#self.QFNNoise = QFNNoise
		
	def getStrength(self):
		strength = 1.0 + self.QFNNoise
		return strength
...
class JPARC_26:
	...
	"""
	def __init__(self, syncpart, lattlength, QDNNoise):
		self.name = "JPARC_26"
		self.syncpart = syncpart
		self.lattlength = lattlength
		#self.QDNNoise = QDNNoise
		
	def getStrength(self):
		strength = 1.0 + self.QDNNoise
		return strength
...
class JPARC_27:
	...
	"""
	def __init__(self, syncpart, lattlength, QFXNoise):
		self.name = "JPARC_27"
		self.syncpart = syncpart
		self.lattlength = lattlength
		#self.QFXNoise = QFXNoise
		
	def getStrength(self):
		strength = 1.0 + self.QFXNoise
		return strength
