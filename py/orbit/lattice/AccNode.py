...
class AccNode(NamedObject, TypedObject, ParamsDictObject):
    ...
    def setLength(self, L=0.0, index=-1):
        """
        Method. Sets the physical length of a node or a
        part if index > 0.
        """
        L = float(L)
        if math.fabs(L) < 1.0e-36:
            L = 0.0
        if index >= 0:
    ...
    def reverseOrder(self):
        ...
        for iPart in range(len(self.__childNodesArr[AccNode.BODY])):
            ...
            for node in self.__childNodesArr[AccNode.BODY][iPart][AccNode.BEFORE]:
                node.reverseOrderNodeSpecific()
            for node in self.__childNodesArr[AccNode.BODY][iPart][AccNode.AFTER]:
                node.reverseOrderNodeSpecific()
