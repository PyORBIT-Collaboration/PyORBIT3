...
def test_grid3d_to_numpy_round_trips_rectangular_grid():
    ...
    script = textwrap.dedent(
        ...
        expected = np.arange(4 * 5 * 6, dtype=np.float64).reshape(4, 5, 6)
        grid = Grid3D(4, 5, 6)
...
def test_bunch_update_from_numpy_is_atomic_on_invalid_input():
...
def test_bunch_to_numpy_gather_on_single_rank():
...
def test_grid3d_from_numpy_uses_xyz_order():
...
def test_missing_numpy_raises_import_error_instead_of_aborting(extension_name):
...
def test_bunch_update_from_numpy_uses_only_local_particle_count(tmp_path):
...
def test_bunch_to_numpy_gathers_particles_across_mpi_ranks(tmp_path):
    ...
    script_path.write_text(
        textwrap.dedent(
            ...
            empty_bunch = Bunch.from_numpy(np.empty((0, 6), dtype=np.float64))
            ...
            with pytest.raises(ValueError, match="root must be between 0 and 2"):
